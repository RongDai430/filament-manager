...
import { formatNumber, jobTotals, remainingPercent, remainingWeight, round2, unitCost } from './domain'
import { loadData, saveData } from './storage'
import type { AppData, FinishInput, Material, MaterialInput, MaterialStatus, PrintJob, PrintJobStatus } from './types'
...
function MaterialRow({ data, material, navigate, setActionModal }: { data: AppData; material: Material; navigate: (page: Page, id?: string) => void; setActionModal: (modal: ActionModal) => void }) {
  const remaining = remainingWeight(material, data.usages)
  const percent = remainingPercent(material, data.usages)
  return <tr><td><button className="material-cell" onClick={() => navigate('material-detail', material.id)}><span className="mini-swatch" style={{ background: colorHex[material.color] || '#c1b9ad' }} /><span><strong>{material.brand} {material.name || material.materialType}</strong><small>{material.materialType} · {material.color} · {formatNumber(material.initialWeightG)}g</small></span></button></td><td><StatusBadge status={material.status} /></td><td><span className="location-cell"><Icon name="location" size={14} />{material.currentLocation}</span></td><td><div className="table-weight"><span>{formatNumber(remaining)}g</span><small>{formatNumber(percent, 1)}%</small><div className="progress-track"><span className={percent < 20 ? 'is-low' : ''} style={{ width: `${percent}%` }} /></div></div></td><td><span className="price-cell">¥{material.price.toFixed(2)}</span><small className="unit-cost">¥{unitCost(material).toFixed(3)} / g</small></td><td><span className="date-cell">{relativeDate(material.updatedAt)}</span></td><td><div className="row-actions"><button className="icon-button subtle" title="查看详情" onClick={() => navigate('material-detail', material.id)}><Icon name="external" size={16} /></button>{material.status === 'STORED' && <button className="small-action" onClick={() => setActionModal({ action: 'mount', material })}>挂载</button>}{material.status === 'MOUNTED' && <button className="small-action" onClick={() => setActionModal({ action: 'unmount', material })}>卸下</button>}{material.status !== 'EMPTY' && <button className="icon-button subtle danger-hover" title="标记用完" onClick={() => setActionModal({ action: 'empty', material })}><Icon name="empty" size={16} /></button>}</div></td></tr>
}

function AddMaterialPage({ onCancel, onCreated }: { onCancel: () => void; onCreated: (input: MaterialInput) => void }) {
  const [form, setForm] = useState<MaterialInput>({ brand: '', materialType: 'PLA', name: '', color: '', initialWeightG: 1000, price: 0, currentLocation: 'AMS A1', note: '' })
  const [errors, setErrors] = useState<Record<string, string>>({})
  const set = (key: keyof MaterialInput, value: string | number) => setForm((current) => ({ ...current, [key]: value }))
  const submit = (event: FormEvent) => {
    event.preventDefault()
    const nextErrors: Record<string, string> = {}
    if (!form.brand.trim()) nextErrors.brand = '必填'
    if (!form.materialType.trim()) nextErrors.materialType = '必填'
    if (!form.color.trim()) nextErrors.color = '必填'
    if (!form.currentLocation.trim()) nextErrors.currentLocation = '必填'
    if (form.initialWeightG <= 0) nextErrors.initialWeightG = '重量必须大于 0'
    if (form.price < 0) nextErrors.price = '价格不能小于 0'
    setErrors(nextErrors)
    if (Object.keys(nextErrors).length === 0) onCreated(form)
  }
  return <form className="form-layout" onSubmit={submit}><div className="form-main panel"><div className="form-section"><div className="form-section-title"><span>01</span><div><h2>基础信息</h2><p>记录这卷耗材的身份信息，之后无需重复录入。</p></div></div><div className="form-grid"><Field label="品牌" required error={errors.brand}><input value={form.brand} onChange={(event) => set('brand', event.target.value)} placeholder="例如 Bambu Lab" /></Field><Field label="材料类型" required error={errors.materialType}><select value={form.materialType} onChange={(event) => set('materialType', event.target.value)}><option>PLA</option><option>PLA+</option><option>PETG</option><option>TPU</option><option>ABS</option><option>ASA</option><option>其他</option></select></Field><Field label="具体名称"><input value={form.name} onChange={(event) => set('name', event.target.value)} placeholder="例如 Basic / Silk（可选）" /></Field><Field label="颜色" required error={errors.color}><div className="input-with-swatch"><span className="input-swatch" style={{ background: colorHex[form.color] || '#d9d1c5' }} /><input value={form.color} onChange={(event) => set('color', event.target.value)} placeholder="例如 哑光黑" /></div></Field></div></div><div className="form-section"><div className="form-section-title"><span>02</span><div><h2>重量与成本</h2><p>后续成本按实际消耗和这卷耗材的单位成本计算。</p></div></div><div className="form-grid"><Field label="初始耗材净重" required suffix="g" error={errors.initialWeightG}><input type="number" min="0.01" step="0.1" value={form.initialWeightG} onChange={(event) => set('initialWeightG', Number(event.target.value))} /></Field><Field label="购入价格" required suffix="CNY" error={errors.price}><input type="number" min="0" step="0.01" value={form.price} onChange={(event) => set('price', Number(event.target.value))} /></Field></div><div className="calculation-hint"><Icon name="cost" size={16} /><span>单位成本将自动计算为</span><strong>¥{form.initialWeightG > 0 ? (form.price / form.initialWeightG).toFixed(3) : '0.000'} / g</strong></div></div><div className="form-section"><div className="form-section-title"><span>03</span><div><h2>当前位置</h2><p>如果现在就挂到机器上，记录它所在的料架或 AMS 槽位。</p></div></div><div className="form-grid"><Field label="当前位置" required error={errors.currentLocation}><input value={form.currentLocation} onChange={(event) => set('currentLocation', event.target.value)} placeholder="例如 AMS A1" /></Field><Field label="备注"><input value={form.note} onChange={(event) => set('note', event.target.value)} placeholder="可选，例如用于模型系列" /></Field></div></div></div><aside className="form-aside"><div className="preview-card"><div className="preview-label">NEW MATERIAL</div><div className="preview-spool" style={{ '--spool-color': colorHex[form.color] || '#d2c9bc' } as React.CSSProperties}><div className="spool-ring"><div className="spool-hole" /></div></div><div className="preview-name"><strong>{form.brand || '品牌名称'}</strong><h3>{form.name || form.materialType || '材料名称'}</h3><span>{form.color || '未填写颜色'}</span></div><div className="preview-divider" /><div className="preview-stat"><span>初始重量</span><strong>{formatNumber(form.initialWeightG)}g</strong></div><div className="preview-stat"><span>购入价格</span><strong>¥{form.price.toFixed(2)}</strong></div><div className="preview-stat"><span>挂载位置</span><strong>{form.currentLocation || '—'}</strong></div></div><div className="info-tip"><Icon name="roll" size={16} /><span>录入后，这卷耗材会保持为一条独立记录。卸下再挂载时，不需要重新录入基础信息。</span></div></aside><div className="form-footer"><button type="button" className="button button-quiet" onClick={onCancel}>取消</button><button type="submit" className="button button-primary button-with-icon"><Icon name="check" size={16} />保存并挂载</button></div></form>
}
...
function FinishPrintPage({ data, job, onCancel, onFinished, onError }: { data: AppData; job: PrintJob; onCancel: () => void; onFinished: (next: AppData) => void; onError: (error: unknown) => void }) {
  const usages = data.usages.filter((usage) => usage.printJobId === job.id)
  const [inputs, setInputs] = useState<FinishInput[]>(() => usages.map((usage) => ({ usageId: usage.id, actualWeightG: null })))
